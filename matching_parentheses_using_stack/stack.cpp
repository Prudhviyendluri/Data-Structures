...
void Stack<Type>::push(const value_type& x){
	if(top_of_stack == stack_size-1){ throw std::overflow_error("Stack Overflow"); } /* Check for overflow. */
	data[++top_of_stack] = x; 
	return; 
}
...
void Stack<Type>::pop(){
	if(top_of_stack == -1){ throw std::underflow_error("Stack Underflow"); }	/* Check for Underflow condition. */
	--top_of_stack;
	return; 
}
